	...
	public static void main(String[] args) {
		...
		for(int j=1; j<4; j++) {
			...
			for(int i=0; i<5; i++) {
				...
				//Sort s = new Sort(Greedy.item);
				System.out.println(Greedy.item.size());
				List tmpItem = Greedy.item;
				System.out.println(tmpItem.size());
				alg1 += Greedy.AlgorithmOne(Greedy.item)/5; //puts to alg1 the average makespan of algorithm 1
				alg2 += Sort.AlgorithmTwo(tmpItem)/5;   // puts to alg2 the average makespan of algorithm 2
			}
